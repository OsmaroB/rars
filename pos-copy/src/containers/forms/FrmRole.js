...
const FrmRole = (props)=>{
    ...
    const updateClick = () => {
        ...
        if(validation.update(newData)){
            dispatch({type: UPDATE_ROLE, role: newData});
    ...
    const addClick = () => {
        ...
        if(validation.data(newData)){
            dispatch({type: ADD_ROLE, role: newData});
    ...
    const removeFinish = (newStatus) => {
        const newData = {...role, status:newStatus};
        dispatch({type: REMOVE_ROLE, role: newData});
                ...
                <div className="d-flex flex-grow-1 min-width-zero">
                    <div className="card-body aling-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero aling-items-md-center">
                        <div className="list-item-heading mb-0 truncate w-80 mb-1 mt-1">
                            ...
                            <h3>{role.name} {role.lastName}</h3>
                        </div>
                    </div>
                    <div className="custom-control custom-checkbox pl-1 align-self-center pr-4">
                        <Button
                            outline
                            color={role.status === 0 ? 'theme-3' : 'light'}
                        ...
                        <Button
                            ...
                            onClick={() => {removeClick()}}
                        >
                            <i className={role.status === 0? 'simple-icon-ban' : 'simple-icon-check'} />
                        </Button>
                    </div>
                </div>
                                ...
                                <FormGroup>
                                    ...
                                    <div className="text-right">
                                        <Button
                                            ...
                                            color="primary"
                                            className="m-1"
                                            onClick={() => {dispatch({type: CLOSE_FORM_ROLE})}}
                                        >
