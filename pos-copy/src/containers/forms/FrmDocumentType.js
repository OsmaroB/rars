...
import validation from "containers/validations/ValidationDocumentType";
import Cookies from 'universal-cookie';

const FrmDocumentType = (props)=>{
    ...
    const updateClick = () => {
        ...
        if(validation.update(newData)){
            dispatch({type: UPDATE_DOCUMENT_TYPE, documentType: newData});
            successMessage('Envio de datos exitoso', 'El item a sido actualizado con exito');
            dispatch({type: CLOSE_FORM_DOCUMENT_TYPE})
        }
    };
    ...
    const addClick = () => {
        ...
        if(validation.data(newData)){
            dispatch({type: ADD_DOCUMENT_TYPE, documentType: newData});
    ...
    const removeFinish = (newStatus) => {
        const newData = {...documentType, status:newStatus};
        dispatch({type: REMOVE_DOCUMENT_TYPE, documentType: newData});
        successMessage('Envio de datos exitoso', newStatus ? 'El item a sido reingresado con exito': 'El item a sido eliminado con exito');
    };

    const removeClick = () => {
        const newStatus = documentType.status === 0 ? 1 : 0;
        if(newStatus === 1){
            Swal.fire({
                ...
                <div className="d-flex flex-grow-1 min-width-zero">
                    <div className="card-body aling-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero aling-items-md-center">
                        <div className="list-item-heading mb-0 truncate w-80 mb-1 mt-1">
                            ...
                            <h3>Nombre de tipo documento{documentType.name}</h3>
                    ...
                    <div className="custom-control custom-checkbox pl-1 align-self-center pr-4">
                        <Button
                            outline
                            color={documentType.status === 0 ? 'theme-3' : 'light'}
                        ...
                        <Button
                            ...
                            onClick={() => {removeClick()}}
                        >
                            <i className={documentType.status === 0? 'simple-icon-ban' : 'simple-icon-check'} />
                        </Button>
                    </div>
                </div>
                                ...
                                <FormGroup>
                                    ...
                                    <div className="text-right">
                                        <Button
                                            ...
                                            color="primary"
                                            className="m-1"
                                            onClick={() => {dispatch({type: CLOSE_FORM_DOCUMENT_TYPE})}}
                                        >
...
export default FrmDocumentType;